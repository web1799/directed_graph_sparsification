...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if or(H{j}.nc >= (length(H{j}.A)-1), total_hierarchy_edges > (4*nnz(H{1}.A)) )
        ...
        warning('MATLAB:CMG:hierarchy:stagnation', 'CMG convergence may be slow due to matrix density. Future versions of CMG will eliminate this problem.');
        break;
    end
...
if (last_level==1)
    H{last_level}.A = laplacian(A)+diag(excD);
end
